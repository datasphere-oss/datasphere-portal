...
import org.unidata.mdm.core.configuration.CoreMessagingDomain;
...
import org.unidata.mdm.system.type.messaging.SystemHeaders;
...
public class SecurityServiceImpl implements SecurityService {
    ...
    */
     ...
     */
    private boolean isCorrectEndpoint(UserInf result, Map<AuthenticationSystemParameter, Object> params) {
        if (result == null || result.getEndpoints() == null) {
            return false;
        }
        return result.getEndpoints().stream().map(org.unidata.mdm.core.type.security.EndpointInf::getName)
                .anyMatch(el ->
                        StringUtils.equals(
                                el,
                                ((EndpointTypeInf) params.get(AuthenticationSystemParameter.PARAM_ENDPOINT)).name()
                        )
                );
    }
     ...
     * @return user or null
     */
    private UserInf processLogin(Map<AuthenticationSystemParameter, Object> params) {

        for (SecurityDataSource sds : dataSources) {

            UserInf result = processLogin(params, sds, false);
            if (Objects.isNull(result)) {
                continue;
            }
     ...
     * @return user or null
     */
    private UserInf processLogin(Map<AuthenticationSystemParameter, Object> params, SecurityDataSource sds, boolean existingProfile) {
        ...
        try {
            ...
            UserInf authUser = authProvider.login(AuthenticationRequestContext.of(params));
            if (authUser != null) {
     ...
     */
    private void processAuthorization(UserInf authUser, SecurityDataSource sds, boolean existingProfile) {
     ...
     * @param existingProfile existing profile
     */
    private void processProfile(UserInf authUser, SecurityDataSource sds, boolean existingProfile) {
     ...
     */
    private SecurityToken createToken(final UserInf user, Map<AuthenticationSystemParameter, Object> params, boolean isInnerToken) {
        ...
        token.setEndpoint((EndpointTypeInf) params.get(AuthenticationSystemParameter.PARAM_ENDPOINT));

        return token;
    }

    private void setTokenAuthority(SecurityToken token, List<Role> roles, List<RightInf> rights, List<SecurityLabelInf> userLabels) {
        ...
        List<SecurityLabelInf> rolesLabels =
        ...
        final Map<String, RightInf> rightsMap = SecurityUtils.createRightsMap(roles);
        final Map<String, Role> rolesMap = SecurityUtils.createRolesMap(roles);

        // Overwrite calculated rights with manually supplied one
        Map<String, RightInf> overwriteRights = SecurityUtils.extractRightsMap(rights);
     ...
     */
    @Override
    public UserInf getUserByToken(String tokenString) {
     ...
     */
    @Override
    public List<RightInf> getRightsByToken(final String tokenString) {
        SecurityToken token = tokenCache.get(tokenString);
        return token == null
                ? Collections.emptyList()
